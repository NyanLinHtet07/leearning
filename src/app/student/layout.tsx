...
function HeaderBar() {
  ...
  return (
    <header className='sticky top-2 bg-white/50 rounded-3xl backdrop-blur-lg drop-shadow-2xl z-50 mx-5  flex h-16.5 items-center gap-2 px-6 justify-between'>
      ...
      {/* logo */}
      <Link href="/student" className="flex items-center gap-3 fex-shrink-0">
      ...
      <nav className='hidden md:flex absolute left-1/2 -translate-x-1/2 items-center gap-8'>
        {
          mainNavigationItems.map((item) => {
            ...
            return (
              ...
              <Link href={item.href} key={item.href} className={`flex items-center uppercase hover:text-primary transition-colors text-xs tracking-wide
