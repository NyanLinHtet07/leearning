...

{
  /* {
                categories.map((cat) => (
                    <Card
                        key={cat.id}
                        className='bg-gradient-to-bl from-shell/50 to-white/50 backdrop-blur-md '>
                        <CardHeader>
                            <h3 className=' text-gray-800 font-semibold text-lg'>{cat.name}</h3>
                            <p className=' text-sm text-primary'>{cat?.description}</p>
                        </CardHeader>
                        <CardContent>
                            <div className=' grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
                                {
                                    cat.courses.map((course) => (
                                        <div className=' px-4 py-2 mx-3 my-2 bg-jade/30 drop-shadow-md shadow cursor-pointer hover:-translate-y-1 rounded-2xl duration-500 transition-all'
                                            key={course.id}>
                                            <div className=' mb-2 pb-2 border-b-2 h-40 border-b-white'>
                                                <h4 className='font-semibold text-lg tracking-wide mb-2'> {course.title} </h4>
                                                <span className=' text-sm'> {course.description
                                                    ?.split(' ')
                                                    .slice(0, 20)
                                                    .join(' ') + '...'} </span>
                                            </div>
                                            <div className=' flex justify-end'>
                                                <Link
                                                    href={`/student/courses/${course.id}`} prefetch
                                                    className=' text-sm text-primary hover:text-primary/60 hover:font-semibold'> Detail</Link>
                                            </div>
